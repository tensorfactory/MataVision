...
In many practical vision tasks, it is often necessary to incorporate inputs from or output data to external systems. MataVision includes a list of I/O types (shown below the image), each designed to seamlessly interact with different types of I/O systems.
...
The following six programs will do the same thing: turn ON the LED when the switch is pressed.<br/>
<br/>
...
On the main page, press the “Browse I/Os” button to open the I/O types page. MataVision supports 14 types of I/O, but for our purposes, we will choose the two simplest types: digital-in and digital-out.
...
<br/>
What we have done is create two I/O objects. Each object has one value. A digital-in type object called digIn13 has an input value also called digIn13. A digital-out type object called digOut15 has an output value also called digOut15.

<br/>
Please note that some I/O objects can have more than one input or output value. For example, an object-classification object will have multiple input values, one for each class. A cat or dog classification object will have a minimum of two input values. Whether the value type is considered “input” or “output” depends on whether the value is input to or output from the program’s perspective. In the case of cat/dog classification, they are all “inputs” to the program.
...
A MataVision program consists of lines of Simple Logic Instructions (SLI). It executes from top to bottom and will continue to loop over these instructions.
<br/>
 <br/><img title="SLI" src="images/sli_3_sections.png" alt="" width="500"><br/> An SLI instruction consists of three segments: an input value on the left, a condition in the middle, and one or more output values on the right. When the condition is evaluated to be TRUE, the value on the left will be assigned to the right.

<img title="IO Monitor." src="images/bottle_cap_detection_system.png" alt="" width="600">

To illustrate how we can use lines of SLI instructions to 
